...
import org.openmrs.api.InvalidObligationValueException;
...
public class DWRPatientService implements GlobalPropertyListener {
	...
	public Map<String, Object> findCountAndPatients(String searchValue, Integer start, Integer length, boolean getMatchCount)
		...
		catch (MissingObligationValueException e) {
			handleSerachError(objectList, resultsMap, e);
		}
		catch (InvalidObligationValueException e) {
			handleSerachError(objectList, resultsMap, e);
		}
		catch (InvalidObligationConfigException e) {
			handleSerachError(objectList, resultsMap, e);
		}
		catch (Exception e) {
	...
	private void handleSerachError(Collection<Object> objectList, Map<String, Object> resultsMap, APIException e) {
		log.error("Obligation releated exception ", e);
		objectList.clear();
		objectList.add(Context.getMessageSourceService().getMessage("Patient.search.error") + " - " + e.getMessage());
		resultsMap.put("count", 0);
		resultsMap.put("objectList", objectList);
